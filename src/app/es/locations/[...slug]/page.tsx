...
export async function generateStaticParams() {
  ...
  if (process.env.ULTRA_FAST_BUILD === 'true' || process.env.FAST_BUILD === 'true') {
    console.log('[Ultra-Fast Build] Skipping Spanish location page pre-generation');
    return [];
  }
...
export async function generateMetadata({
  ...
  const title = service
    ? `${service
        ...
        .join(' ')} in ${baseTitle}`
    : `${baseTitle} Abogados | Vasquez Law Firm`;

  const description = service
    ? `Expert ${service.replace(/-/g, ' ')} services in ${cityName}, ${stateUpper}. Free consultation: 1-844-YO-PELEO`
    : `Top-rated attorneys serving ${cityName}, ${stateUpper}. Inmigración, personal injury, criminal defense, and workers' compensation. Free consultation: 1-844-YO-PELEO`;
