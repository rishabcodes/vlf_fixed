...
export class GHLMCPClient {
  ...
  constructor() {
    ...
    if (!this.pitToken) {
      logger.warn('GHL MCP Client: No PIT token configured');
    }
  }
  ...
  async searchContactByEmail(email: string): Promise<any | null> {
    ...
    try {
      ...
      logger.error('Error searching contact by email:', error);
      return null;
    }
  }
  ...
  async searchContactByPhone(phone: string): Promise<any | null> {
    ...
    try {
      ...
      logger.error('Error searching contact by phone:', error);
      return null;
    }
  }
  ...
  async createOrUpdateContact(data: ContactData, chatSummary?: ChatSummary): Promise<{ id: string; created: boolean; updated: boolean }> {
    ...
    try {
      ...
      if (existingContact) {
        ...
        logger.info(`[GHL] Found existing contact: ${existingContact.id}`);
        ...
        // Create new contact
        logger.info('[GHL] Creating new contact');
      ...
      logger.error('Failed to create or update contact:', error);
      // Queue for retry
...
`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (errorString.includes('429') || errorString.includes('rate limit')) {
          ...
          logger.info(`[GHL] Rate limited, waiting ${delay}ms before retry ${attempt}/${maxAttempts}`);
        ...
        if (errorString.includes('401') || errorString.includes('403')) {
          ...
          logger.error('[GHL] Authentication error - check PIT token');
        ...
        if (attempt < maxAttempts) {
          ...
          logger.info(`[GHL] Request failed, retrying in ${delay}ms (${attempt}/${maxAttempts})`);
          await this.sleep(delay);
          continue;
        }
        ...
        if (attempt < maxAttempts) {
          const delay = this.retryDelays[Math.min(attempt - 1, this.retryDelays.length - 1)];
          logger.info(`[GHL] Network error, retrying in ${delay}ms (${attempt}/${maxAttempts})`);
          await this.sleep(delay);
          continue;
        }
      }
  ...
  private queueFailedRequest(request: any): void {
    ...
    logger.info(`[GHL] Queued failed request. Queue size: ${this.failedRequestQueue.length}`);
  }
  ...
  async processFailedRequests(): Promise<void> {
    ...
    logger.info(`[GHL] Processing ${this.failedRequestQueue.length} failed requests`);
      ...
      try {
        ...
        switch (operation) {
          case 'createOrUpdateContact':
            ...
            break;
          default:
            logger.warn(`[GHL] Unknown queued operation: ${operation}`);
        }
      } catch (error) {
        logger.error('[GHL] Failed to process queued request:', error);
  ...
  async addContactTags(contactId: string, tags: string[]): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Failed to add contact tags:', error);
      return false;
    }
  }
  ...
  async createContactNote(contactId: string, noteBody: string): Promise<string | null> {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        logger.info(`[GHL] Note created for contact ${contactId}: ${noteId}`);
        return noteId;
      }
      
      return null;
    } catch (error) {
      logger.error('Failed to create contact note:', error);
      return null;
    }
  }
  ...
  async getContactNotes(contactId: string): Promise<any[]> {
    ...
    try {
      ...
      logger.error('Failed to get contact notes:', error);
      return [];
    }
  }
  ...
  async updateContactNote(contactId: string, noteId: string, noteBody: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Failed to update contact note:', error);
      return false;
    }
  }
  ...
  private async executeRestRequest(method: string, path: string, data?: any): Promise<MCPResponse> {
    ...
    try {
      ...
      logger.error(`[GHL] REST request failed: ${method} ${path}`, error);
      return {
        success: false,
  ...
  public async executeTool(tool: string, input: Record<string, any>): Promise<MCPResponse> {
    ...
    try {
      ...
      logger.info(`[MCP] Executing tool: ${tool}`, {
      ...
      if (responseText.startsWith('event:')) {
        ...
        try {
          ...
          logger.error('[MCP] Failed to parse SSE response:', jsonData);
      ...
      if (data.error) {
        ...
        if (parsedResult.content && Array.isArray(parsedResult.content)) {
          ...
          if (textContent) {
            try {
              ...
              logger.info('[MCP] Could not parse nested result:', e);
            }
          }
        }
        ...
        if (result.success && result.data) {
          logger.info(`[MCP] Success for ${tool}:`, {
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.info('[MCP] Falling back to mock data');
        return {
          success: false,
